...
def run():
    ...
    log(f"Telegram filters: Volume >= {TELEGRAM_MIN_VOLUME:,}, Price >= ₹{TELEGRAM_MIN_PRICE:,.2f}")
    ...
    if not state.get("baseline_initialized", False):
        ...
        save_state(state)
        commit_state()
        log(f"Daily filtered baseline initialized with {len(current_symbols)} symbols. No alerts sent on baseline run.")
        return
