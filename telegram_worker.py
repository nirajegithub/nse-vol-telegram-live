...
# Fixed Telegram alert filters.
# These are independent of Streamlit user filters.
...
def apply_telegram_filters(rows):
    """Apply fixed Telegram filters before baseline/detection."""
    filtered = []
    ...
    for row in rows:
        try:
            ...
            if not symbol:
                continue
            if volume < TELEGRAM_MIN_VOLUME:
                continue
            if price < TELEGRAM_MIN_PRICE:
                continue
...
def commit_state():
    ...
    subprocess.run(
        ["git", "commit", "-m", "Update Telegram notification state"],
        check=True,
    )
...
def run():
    ...
    # Apply fixed Telegram filters BEFORE baseline and new-symbol detection.
    ...
    # First valid run of each trading day establishes a FILTERED baseline.
    # No Telegram alerts are sent on the baseline run.
    if not state.get("baseline_initialized", False):
        ...
        log(
            ...
            f"{len(current_symbols)} symbols. No alerts sent on baseline run."
        )
        return

    # Detect only NEW symbols from the filtered dataset.
    new_rows = []
    ...
    for row in new_rows:
        ...
        try:
            ...
            # Failed alerts are not added to state and can retry next run.
