...
def load_state(today):
    ...
    if state.get("date") != today:
        return default
    return {"date": today, "notified_symbols": sorted(set(state.get("notified_symbols", []))), "baseline_initialized": bool(state.get("baseline_initialized", False))}
...
def run():
    ...
    rows, _ = fetch_volume_gainers()
    log(f"NSE Volume Gainers rows: {len(rows)}")
    ...
    # First valid run of each trading day establishes a baseline.
    # This prevents dozens of alerts at 09:15 for stocks already present
    # in the Volume Gainers list when monitoring starts.
    if not state.get("baseline_initialized", False):
        ...
        save_state(state)
        commit_state()
        log(f"Daily baseline initialized with {len(current_symbols)} symbols. No alerts sent on baseline run.")
        return
    ...
    log(f"New symbols: {len(new_rows)}")
    state_changed = False
