...
# GitHub schedule checks are intended to run from 09:22 through 15:17 IST.
# Keep 09:15 as the normal market-window start and allow the final 15:17 run.
...
def create_nse_session():
    ...
    time.sleep(0.5)
    session.get(f"{NSE_BASE_URL}/market-data/volume-gainers", timeout=REQUEST_TIMEOUT)
    time.sleep(0.5)
    return session
...
def fetch_volume_gainers():
    ...
    response = session.get(VOLUME_GAINERS_URL, timeout=REQUEST_TIMEOUT)
...
def apply_telegram_filters(rows):
    ...
    for row in rows:
        try:
            ...
            if symbol and volume >= TELEGRAM_MIN_VOLUME and price >= TELEGRAM_MIN_PRICE:
                filtered.append(row)
...
def load_holidays():
    ...
    if data.get("market") != "CM":
        raise RuntimeError("nse_holidays.json must contain market='CM'.")
...
def load_state(today):
    default = {"date": today, "notified_symbols": [], "baseline_initialized": False}
    ...
    if state.get("date") != today:
        log(f"New trading date detected. Resetting state from {state.get('date')} to {today}.")
        return default

    return {
        "date": today,
        "notified_symbols": sorted(set(state.get("notified_symbols", []))),
        "baseline_initialized": bool(state.get("baseline_initialized", False)),
    }
...
def build_message(row, detected_at):
    ...
    return (
        ...
        f"📈 <b>Price Change:</b> {fmt_percent(row.get('pChange'))}\n"
        f"📊 <b>Volume:</b> {fmt_number(row.get('volume'))}\n"
        f"🔥 <b>Volume Spike vs 1wk:</b> {fmt_percent(row.get('week1volChange'))}\n"
        f"🔥 <b>Volume Spike vs 2wk:</b> {fmt_percent(row.get('week2volChange'))}\n\n"
        f"🕒 <b>Detected:</b> {detected_at} IST\n\n"
        "⚠️ <b>Disclaimer:</b>\n"
        "This is an automated NSE market-data alert for informational purposes only. "
        "It is not investment advice, a buy/sell recommendation, or a trading signal. "
        "Please do your own research and risk assessment before making any investment "
        "or trading decision."
    )
...
def commit_state():
    ...
    subprocess.run(["git", "commit", "-m", "Update Telegram notification state"], check=True)
...
def run():
    ...
    log(f"Current time: {now:%Y-%m-%d %H:%M:%S} IST")

    if not is_trading_day(now):
        return

    if not is_monitoring_window(now):
        log("Outside 09:15-15:17 IST monitoring window - skipping.")
        return
    ...
    log(f"NSE Volume Gainers rows from NSE: {len(rows)}")

    filtered_rows = apply_telegram_filters(rows)
    log(
        f"Telegram filters: Volume >= {TELEGRAM_MIN_VOLUME:,}, "
        f"Price >= ₹{TELEGRAM_MIN_PRICE:,.2f}"
    )
    log(f"Eligible Telegram rows: {len(filtered_rows)}")
    ...
    # First successful run of each trading day creates the baseline.
    # Existing qualifying symbols are deliberately not alerted on that run.
    if not state.get("baseline_initialized", False):
        ...
        commit_state()
        log(
            f"Daily filtered baseline initialized with {len(current_symbols)} symbols. "
    ...
    for row in new_rows:
        symbol = str(row.get("symbol") or "").strip().upper()
        try:
            send_telegram(build_message(row, detected_at))
            notified.add(symbol)
            state_changed = True
            log(f"Telegram sent: {symbol}")
        except Exception as exc:
            log(f"Telegram failed for {symbol}: {exc}")
